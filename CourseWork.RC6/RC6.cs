...
namespace CourseWork.RC6
{
    public class RC6
    {
        ...
        public void GenerateKey(int Long, byte[] mainKey = null)
        {
            if (mainKey == null)
            {
        ...
        public byte[] Encrypt(byte[] byteText)
        {
            ...
            for (i = 0; i < text.Length; i = i + 16)
            {
                ...
                for (int j = 1; j <= R; j++)
                {
                    uint t = LeftShift((B * (2 * B + 1)), (int)(Math.Log(_w, 2)));
                    uint u = LeftShift((D * (2 * D + 1)), (int)(Math.Log(_w, 2)));
